...
if (!isset($_SESSION['urutan_soal'])) {
    ...
    // Cek apakah mapel mengandung kata "Bahasa Indonesia" atau "B. Indonesia"
    $is_bindo = (stripos($mapel_aktif, 'bahasa indonesia') !== false || stripos($mapel_aktif, 'b. indonesia') !== false);
    
    // Hanya acak soal JIKA BUKAN Bahasa Indonesia
    if (!$is_bindo) {
        shuffle($daftar_soal); 
    }
...
<body oncontextmenu="return false;" oncopy="return false;" onpaste="return false;" onkeydown="return mencegahAksi(event);">
    ...
    <div id="modal-zoom-gambar" style="display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.9); z-index: 100005; justify-content: center; align-items: center; flex-direction: column;">
        <span onclick="tutupZoomGambar()" style="position: absolute; top: 60px; right: 20px; color: white; font-size: 35px; cursor: pointer; font-weight: bold; background: rgba(0,0,0,0.5); width: 45px; height: 45px; border-radius: 50%; display: flex; align-items: center; justify-content: center;">&times;</span>
        <img id="gambar-zoom-aktif" src="" style="max-width: 85%; max-height: 75vh; object-fit: contain; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.5);">
    ...
    <script>
        ...
        document.getElementById('btn-fullscreen').addEventListener('click', () => {
            ...
            // Eksekusi Fullscreen (Ditambahkan catch agar tidak memblokir jika HP menolak)
            if (elem.requestFullscreen) { 
                elem.requestFullscreen().catch(err => console.log("Fullscreen ditolak")); 
            ...
            // PERBAIKAN: Gunakan variabel asli bawaan kode Bapak
            overlay.style.display = 'none';
            kontenUjian.style.display = 'block';
            
            // Aktifkan sensor setelah setengah detik agar transisi mulus
            setTimeout(() => { isUjianAktif = true; }, 500); 
        });
        ...
        function catatPelanggaran(jenis) {
            ...
            isSubmitting = true; 
                ...
                // PERBAIKAN: Langsung eksekusi submit tanpa jeda setTimeout
                document.getElementById('form-ujian').submit(); 
            } else {
        ...
        document.addEventListener('touchstart', (e) => {
            // Tambahkan isModalOpen agar sensor ini mati SEMENTARA saat gambar di-zoom
        ...
        setInterval(() => {
            ...
            .then(data => {
                if(data.status === 'stop') {
                    // PERBAIKAN: Paksa submit jawaban, BUKAN sekadar dialihkan
                    isSubmitting = true;
                    document.getElementById('form-ujian').submit(); 
                }
            }).catch(() => {});
        ...
        const hitungMundur = setInterval(() => {
            if (sisaDetik <= 0) {
                ...
                isSubmitting = true;
                document.getElementById('layar-hitam').innerHTML = "<h2>Waktu Ujian Habis!</h2><p>Menyimpan jawaban secara otomatis...</p>";
                document.getElementById('layar-hitam').style.display = 'flex';
                // PERBAIKAN: Langsung eksekusi submit tanpa jeda setTimeout
                document.getElementById('form-ujian').submit();
            } else {
                // ... (biarkan bagian else tetap sama untuk menghitung jam, menit, detik)
